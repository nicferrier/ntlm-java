...
public class HttpForwarder extends Thread {
    ...
    public HttpForwarder(Properties props, int lport) throws IOException {
        ...
        delegateClient
            ...
            .setProxy(props.getProperty(Main.PROXY_DELEGATE_HOST_NAME),
                      Integer.parseInt(props.getProperty(Main.PROXY_DELEGATE_HOST_PORT)));
        delegateClient
            .getState()
            .setProxyCredentials(new AuthScope(AuthScope.ANY),
                                 new NTCredentials(props.getProperty(Main.PROXY_DELEGATE_USERNAME),
                                                   props.getProperty(Main.PROXY_DELEGATE_PASSWORD),
                                                   InetAddress.getLocalHost().getHostName(),
                                                   props.getProperty(Main.PROXY_DELEGATE_DOMAIN)));
        noDelegateClient = new HttpClient(manager);
    }

    static List stripHeadersIn
        = Arrays.asList(new String[] { "Content-Type",
                                       "Content-Length", "Proxy-Connection" });

    static List stripHeadersOut
        = Arrays.asList(new String[] { "Proxy-Authentication",
                                       "Proxy-Authorization" });
